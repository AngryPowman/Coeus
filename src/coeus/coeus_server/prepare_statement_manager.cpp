...
void PrepareStatementManager::doPrepareStatement()
{
    ...
    PREPARE_STATEMENT(STMT_LOAD_CHARACTER,       "SELECT character_id, character_type, nickname, gender, charicteristic, family_type, story_type FROM player_characters WHERE userGuid = ?;", EXECUTOR_SYNC);
