...
void PrepareStatementManager::doPrepareStatement()
{
    ...
    PREPARE_STATEMENT(STMT_LOAD_CHARACTER,       "SELECT character_id, character_type, nickname, gender, charicteristic, family_type, story_type FROM player_characters WHERE user_guid = ?;", EXECUTOR_SYNC);
    PREPARE_STATEMENT(STMT_INSERT_NEW_CHARACTER, "REPLACE INTO player_characters(user_guid, character_id, character_type, nickname, gender, charicteristic, family_type, story_type)"
        " VALUES (?, ?, ?, ?, ?, ?, ?, ?);", EXECUTOR_SYNC);
    PREPARE_STATEMENT(STMT_SAVE_CHARACTER,       "REPLACE INTO player_characters(user_guid, character_id, character_type, nickname, gender, charicteristic, family_type, story_type)"
        " VALUES (?, ?, ?, ?, ?, ?, ?, ?);", EXECUTOR_SYNC);
}