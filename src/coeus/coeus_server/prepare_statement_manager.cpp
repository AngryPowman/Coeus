...
void PrepareStatementManager::doPrepareStatement()
{
    ...
    PREPARE_STATEMENT(STMT_LOAD_CHARACTER,       "SELECT character_id, character_type, nickname, gender, charicteristic, family_type, story_type, last_login, level, hp, max_hp, exp, next_exp, power FROM player_characters WHERE user_guid = ?;", EXECUTOR_SYNC);
